...
export function DataTable<TData, TValue>({
  ...
  // Add a serial number column to the beginning of the columns array
  ...
  const getTableDataForExport = () => {
    ...
    const rows = table.getFilteredRowModel().rows.map(row =>
      visibleColumns.map(column => {
        ...
        // Handle the new serial number column for export
        if (columnId === 'serialNumber') {
          return table.getFilteredRowModel().rows.indexOf(row) + 1;
        }
