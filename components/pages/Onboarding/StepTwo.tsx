...
export default function StepTwo({ formData, setFormData, goNext, goBack }: any) {
  ...
  const isDisabled = !inputs.name || !inputs.email || !inputs.phone || !inputs.teacherPref;
  ...
  return (
    ...
    >
        ...
        <div className="">
          ...
          <div className="flex gap-3 justify-start">
            {[1, 2, 3, 4, 5].map((num) => (
              <label
                key={num}
                className={`flex items-center text-xs justify-center w-10 h-10 p-3 rounded-full cursor-pointer border transition ${inputs.studentCount === String(num)
          ...
          <div className="grid grid-cols-2 gap-6">
            {["male", "female"].map((pref) => (
              <div
                ...
                className={`cursor-pointer border rounded-xl overflow-hidden shadow-sm transition-all ${inputs.teacherPref === pref
                    ? "bg-primary text-white border-primary"
              ...
              >
                <div className="w-32 flex flex-col-reverse mx-auto pt-2 items-center justify-center">
                  <img
                    ...
                    alt={pref}
                    className={`-mb-2 transition-all ${inputs.teacherPref === pref ? "grayscale-0" : "grayscale"
                      }`}
                  />
      ...
      <div className="flex justify-between mt-6">
        ...
        <button
          className={`px-4 py-2 rounded-sm font-medium ${isDisabled
