...
export default function StudentCreatedEmailPage() {
  ...
  useEffect(() => {
    async function fetchTemplate() {
      try {
        const res = await fetch("/api/email?event=student_created");
  ...
  return (
    <div className="p-6">
      <h1 className="text-xl font-semibold mb-4">Edit Student Created Email</h1>
      <EmailEditor template={template}
        requiredFields={["name", "educationMail", "userId", "password"]}
      />
    </div>
  );
}
