...
export default function ForgotPasswordEmailPage() {
  ...
  useEffect(() => {
    async function fetchTemplate() {
      try {
        const res = await fetch("/api/email?event=fee_reminder");
  ...
  return (
    <div className="p-6 bg-white dark:bg-[#122031] rounded-lg shadow-lg">
      <h1 className="text-xl font-semibold mb-4">Edit Fee Reminder Email Template</h1>

      {/* Pass requiredFields */}
      <EmailEditor
        template={template}
        requiredFields={["name", "dueDate", "extendedDueDate"]}
      />
    </div>
  );
}
